...
<form action="task3.php"  method="GET" id="task3">
...
function sqlQuery($con, $country, $page = 1){
	...
	$result = mysqli_query($con, $query) or die("Ошибка " . mysqli_error($con));
	echo "<div class='wraplist'><ul>";
	while ($row = mysqli_fetch_row($result)){
		echo "<li>$row[0]</li>";
	}
	echo "</ul>";
	if(truePage($con,$country,$page) && $page == 1):  echo "<button class='nextpage' id='nextpage-task3'>Next</button>"; 
	elseif(truePage($con,$country,$page) == false && $page > 1): echo "<button class='prevpage' id='prevpage-task3'>Prev</button>";
	elseif(truePage($con,$country,$page) && $page > 1): 
		echo "<button class='prevpage' id='prevpage-task3'>Prev</button>";
		echo "<button class='nextpage' id='nextpage-task3'>Next</button>";
	endif;
	echo "<script>var variable = $page</script></div>";
